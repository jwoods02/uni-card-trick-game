    ...
    while True:
        ...
        if (len(chosen_card_piles) == 3 and
            ...
            if ((0 <= chosen_card_piles[0] <= 4) and
                ...
                print()
                print("DEBUG:", chosen_card_piles[0], chosen_card_piles[1])
                print()

                if ((chosen_card_piles[0] != [""]) and
                    (chosen_card_piles[1] != [""])):
...
def find_number_of_piles_remaining(card_piles):
    number_of_piles_remaining = 0
    for pile in card_piles:
        if pile != [""]:
            number_of_piles_remaining += 1

    return number_of_piles_remaining


def find_number_of_cards_remaining(card_pile):
    number_of_cards_remaining = 0
    for card in card_pile:
        if card != "":
...
card_piles = [deck_fragment[0:2], deck_fragment[2:4], deck_fragment[4:6],
              deck_fragment[6:8], deck_fragment[8:11]]
...
while find_number_of_piles_remaining(card_piles) > 2:
...
if find_number_of_piles_remaining(card_piles) is not 1:
